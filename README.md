...
[] in expo - note that next is not used here so not related to page v app routers:

- expo-apple-authentication
- expo-crypto
- expo-image

[] ANON_KEY - sometimes is without supabase
[] nextjs/src/app/env.mjs - quite different but likely new t3 version - only inserted client VARS
[] make sure setup aligns to supabase auth manual setup: https://supabase.com/docs/guides/auth/auth-helpers/nextjs#manual-configuration
[] next-auth removal
[] install node dependencies @ root - `pnpm i`
[] push the prisma schema @ root: `pnpm -F db db:push` can just run `pnpm db db:push` from root after initial
